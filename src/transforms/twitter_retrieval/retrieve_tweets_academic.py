...
# from utils.pyspark_utils import DataType, SaveMode, configure, transform_df, transform, Input, Output, limit_rate, asynchronous, threaded
...
#         T.StructField('referenced_tweets', T.ArrayType(
#             T.StructType([
#                 T.StructField('type', T.StringType()),
#                 T.StructField('id', T.LongType()),
#             ])
#         )),
#         T.StructField('context_annotations', T.ArrayType(
#             T.StructType([
#                 T.StructField('domain', T.StructType([
#                     T.StructField('id', T.StringType()),
#                     T.StructField('name', T.StringType()),
#                     T.StructField('description', T.StringType()),
#                 ])),
#                 T.StructField('entity', T.StructType([
#                     T.StructField('id', T.StringType()),
#                     T.StructField('name', T.StringType()),
#                 ])),
#             ])
...
# RETRY_ERRORS = [400, 429, 443, 444 , 500, 599]
# TWEET_KEYS_REQ = ['id', 'author_id', 'source','text','conversation_id', 'created_at', 'entities', 'in_reply_to_user_id', 'lang', 'referenced_tweets', 'context_annotations']
# TWEET_KEYS = ['id', 'text', 'conversation_id', 'created_at', 'in_reply_to_user_id', 'lang']


# @limit_rate(limit=1500, interval_in_secods=900)
# def get_tweets(twitter_api_v1, user_id, next_page_token):
#     return twitter_api_v1.get_users_tweets(
#         id=user_id,
#         tweet_fields = TWEET_KEYS_REQ,
#         exclude=['retweets'],
#         max_results=100,
#         pagination_token=next_page_token
#     )

# @threaded(max_workers=5)
# def retrieve_tweets(row, tokens):
...
#                     tmp_result['entities'] = tweet.get('entities')
...
#         .filter(F.col("friends_count") <= 600)
#         .filter(F.col("friends_count") > 500)
#         .repartition(64)
...
#         df
#         .rdd
#         .mapPartitions(lambda partition: retrieve_tweets(partition, tokens))
#         .toDF(SCHEMA)
#     )
